...
func TestMatrix_ToRow(t *testing.T) {
	a := FromVector([]int{1, 2, 3}, true)
	r := a.ToRow()
	if len(r) != 3 || r[0] != 1 || r[1] != 2 || r[2] != 3 {
		t.Error("output row doesn't correspond")
	}
}

func TestTranspose_Any(t *testing.T) {
	a := [][]int{
		{1, 1},
		{-4, -8},
		{2, 0},
	...
	b := Transpose(FromTable(a)).ToTable()

	for i := range len(a) {
		for j := range len(a[0]) {
			if a[i][j] != b[j][i] {
				t.Error("transposition is wrong")
			}
		}
	}
}

func TestTranspose_Column(t *testing.T) {
	a := FromVector([]int{1, 2, 3, 4}, true)
	b := Transpose(a)

	if !b.IsEqualTo(FromVector([]int{1, 2, 3, 4}, false)) {
		t.Error("b should be a row corresponding to a transposed")
	}
}
